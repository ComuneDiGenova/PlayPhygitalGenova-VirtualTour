...
// Mod By Andrea Giuliano
//

Shader "Skybox/PanoramicLerp" {
Properties { 
    _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
    [Gamma] _Exposure ("Exposure", Range(0, 8)) = 1.0
    _Rotation ("Rotation", Range(0, 360)) = 0
    _MainOffset ("MainOffset", Range(0.0, 1.0)) = 0
    _SecondaryOffset ("SecondaryOffset", Range(0.0, 1.0)) = 0
    [NoScaleOffset] _MainTex ("Main Spherical  (HDR)", 2D) = "grey" {}
    [NoScaleOffset] _SecondaryTex ("Secondary Spherical  (HDR)", 2D) = "grey" {}
...
#endif
            float2 tcp = tc + float2(_MainOffset,0);
            float2 tcs = tc + float2(_SecondaryOffset,0);
            half4 tex = tex2D (_MainTex, tcp);
            half3 c = DecodeHDR (tex, _MainTex_HDR);
            half4 tex2 = tex2D (_SecondaryTex, tcs);
            half3 c2 = DecodeHDR (tex2, _SecondaryTex_HDR);
            half3 cl = lerp(c,c2,_Lerp);
            c = cl * _Tint.rgb * unity_ColorSpaceDouble.rgb;
            c *= _Exposure;
            return half4(c, 1);
        }
        ENDCG
