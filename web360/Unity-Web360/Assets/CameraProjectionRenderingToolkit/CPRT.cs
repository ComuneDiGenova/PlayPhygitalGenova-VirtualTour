...
namespace CameraProjectionRenderingToolkit
{
    ...
    public class CPRT : MonoBehaviour
    {
        ...
        public void OnValidate()
        {
            ...
            {//v1.6, if we select it in the editor
                RefreshEffect();
                RefreshViewport();
            }
        }
        ...
        public void CheckViewportSize()
        {
...

            {
                ...
                if (rtW > 0 && rtH > 0 && cam.nearClipPlane > 0.0f && cam.farClipPlane > cam.nearClipPlane)
                {//v1.6
                    switch (projectionType)
                    {
                        case CPRTType.ObliqueOrthographic:
                            { //if (projectionType==CPRTType.ObliqueOrthographic&&cam.orthographic) {
                                if (cam.orthographicSize != 0.0f)
                                {//v1.6
                                    Matrix4x4 mat;
        ...
        public bool CheckResources()
        {
            if (projectionType != (CPRTType)0)
            {
                ...
                if (isSupported)
                {//control shader and create material 
                    if (cprtShader == null)
                    {
