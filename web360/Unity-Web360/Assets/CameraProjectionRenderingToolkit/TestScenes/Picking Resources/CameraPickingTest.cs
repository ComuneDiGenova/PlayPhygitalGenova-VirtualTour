...
public class CameraPickingTest : MonoBehaviour
{
    ...
    void Update()
    {
        if (init)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (IsScreenPointInViewport(Input.mousePosition))
                {//if the mouse is in the viewport
                    ...
                    foreach (Collider collider in clickables)
                    {//checking every clickable object for collision with the ray
                        RaycastHit hitInfo;
            ...
            if (heldObject)
            {
                ...
                {//holding the object
                    ...
                    if (mz != 0.0f)
                    {//modifying object distance with the mousewheel
    ...
    private void DebugClickAt(Vector3 mousePosition)
    {//Drawing debug lines
        Ray ray;
